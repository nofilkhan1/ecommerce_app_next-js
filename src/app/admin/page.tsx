...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Products', value: stats.total, icon: '📦', color: 'bg-[#eff6ff]', change: '+0 this month' },
    { label: 'Categories', value: stats.categories, icon: '🏷️', color: 'bg-[#f0fdf4]', change: `${stats.categories} active` },
    { label: 'Total Stock', value: stats.stock, icon: '📊', color: 'bg-[#fefce8]', change: `${stats.outOfStock} out of stock` },
    { label: 'Inventory Value', value: `PKR ${stats.value.toLocaleString()}`, icon: '💰', color: 'bg-[#fdf4ff]', change: `${stats.lowStock} low stock` },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {Array.from({ length: 4 }).map((_, i) => (
            <Card key={i}><CardContent><CardSkeleton /></CardContent></Card>
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
              ...
              <CardContent>
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-[#111] mt-2">{stat.value}</p>
                    <p className="text-xs text-[#9ca3af] mt-1.5">{stat.change}</p>
                  </div>
      ...
      {/* Quick Actions */}
      <div>
        <h2 className="text-sm font-semibold text-[#111] mb-4">Quick Actions</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Link href="/admin/products" className="group">
            <Card hover>
              <CardContent>
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[#111] flex items-center justify-center text-white group-hover:scale-105 transition-transform">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#111]">Add Product</p>
          ...
          <Link href="/admin/products" className="group">
            <Card hover>
              <CardContent>
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-[#f3f4f6] flex items-center justify-center text-[#374151] group-hover:scale-105 transition-transform">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#111]">Manage Products</p>
            ...
            <Card hover>
              <CardContent>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#111]">View Store</p>
            ...
            <Card hover>
              <CardContent>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-[#111]">Settings</p>
      ...
      <Card>
        <div className="px-6 py-4 border-b border-[#f3f4f6] flex items-center justify-between">
          <h2 className="text-sm font-semibold text-[#111]">Recent Products</h2>
          <Link href="/admin/products" className="text-xs font-medium text-[#6b7280] hover:text-[#111] transition-colors">
            View all
          </Link>
        </div>
        {loading ? (
              ...
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="h-2 bg-[#e5e7eb] rounded animate-pulse w-1/4" />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="p-12 text-center">
            <p className="text-sm text-[#9ca3af]">No products yet.</p>
          </div>
        ) : (
          <Table>
            <TableHeader>
              ...
              {products.slice(0, 5).map((p) => {
                ...
                return (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      <span className="text-[#6b7280]">{p.stock}</span>
                    </TableCell>
                    <TableCell>
